...
#include "cmsis_gcc.h"
    ...
    #define m_STK_RVR_RELOAD (SysTick->LOAD & SysTick_LOAD_RELOAD_Msk) //читаем из STK_RVR поле RELOAD
    #define m_STK_RVR_RELOAD_SET(RELOAD) { SysTick->LOAD = \
    ...
    #define m_STK_CVR_CURRENT (SysTick->VAL & SysTick_VAL_CURRENT_Msk) //читаем из STK_CVR поле CURRENT
    #define m_STK_CVR_CURRENT_CLEAR { SysTick->VAL &= ~SysTick_VAL_CURRENT_Msk; } //чистим счетчик STK_CVR_CURRENT актом записи, тем самым заставляя SysTick перейти к новому циклу, что также чистит флаг COUNTFLAG в STK_CSR. Это не вызывает прерывание (тот случается при переходе счетчика от 1 к 0).

    //3. Работу с STK_CALIB опускаем - для STM32F0 он какой-то не рабочий (только чтение)
    #define m_STK_CALIB (SysTick->CALIB) //хотя, прочитаем.

    //4. Работаем с регистром управления и состояния STK_CSR (и консолидировано, и отдельно по полям)
    #define m_STK_CSR_COUNTFLAG ( (SysTick->CTRL & SysTick_CTRL_COUNTFLAG_Msk) >> SysTick_CTRL_COUNTFLAG_Pos ) //Вернет флаг - 1 если таймер досчитал до 0 с момента последнего считывания (т.е. такое чтение сбросит этот флаг). (Флаг сдвигается вправо на 0-й бит).
    #define m_STK_CSR_CLKSOURCE ( (SysTick->CTRL & SysTick_CTRL_CLKSOURCE_Msk) >> SysTick_CTRL_CLKSOURCE_Pos ) //Вернет флаг - источник тактиторания: 0 = Внешний, 1 = Тактировщик ЦП (HCLK). (Флаг сдвигается вправо на 0-й бит).
    #define m_STK_CSR_TICKINT   ( (SysTick->CTRL & SysTick_CTRL_TICKINT_Msk)   >> SysTick_CTRL_TICKINT_Pos )   //Вернет флаг - генерировать ли прерывание SysTick по достижению счетчика STK_CVR ноля: 0 = Нет, 1 = Да. (Флаг сдвигается вправо на 0-й бит).
    #define m_STK_CSR_ENABLE    ( (SysTick->CTRL & SysTick_CTRL_ENABLE_Msk)     >> SysTick_CTRL_ENABLE_Pos )   //Вернет флаг - запущен ли счетчик SysTick: 0 = Нет, 1 = Да. (Флаг сдвигается вправо на 0-й бит).
    #define m_STK_CSR           (SysTick->CTRL) //Вернет значение всего регистра, как есть.

    #define m_STK_CSR_CLKSOURCE_SET(CLKSOURCE) { SysTick->CTRL = \
    ...
    #define m_STK_CSR_TICKINT_SET(TICKINT) { SysTick->CTRL = \
    ...
    #define m_STK_CSR_ENABLE_SET(ENABLE) { SysTick->CTRL = \
    ...
    #define m_ASM_DataSynchronizationBarrier { __DSB(); } //выполняем инструкцию барьера сразу после настройки и запуска SysTick, дабы убедиться, что он точно стартует в этой точке (команда из #include "cmsis_gcc.h")

    #define m_STK_CSR_SET(CLKSOURCE, TICKINT, ENABLE) { SysTick->CTRL = \
        ...
        m_ASM_DataSynchronizationBarrier \
